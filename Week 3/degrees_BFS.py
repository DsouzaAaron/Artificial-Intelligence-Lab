...
def shortest_path(source, target):
    ...
    """
    queue = QueueFrontier()
    queue.add(Node(source,None,0,None))
        ...
        for movie_id,person_id in neighbors_for_person(node.state):
            if person_id == target:
                ...
                while node.parent is not None:
                    ...
                    node = node.parent
                return path
            elif person_id not in visited and not queue.contains_state(person_id):
                queue.add(Node(person_id,node,0,movie_id))

    # for movie_id, person_id in neighbors_for_person(source):
    #     queue.add(Node(person_id,None,0,movie_id))
